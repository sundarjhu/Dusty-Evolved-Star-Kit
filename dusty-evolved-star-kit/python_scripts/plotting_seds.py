...
if len(a) == 21:
    ...
    averages = ['Median', np.median(a['L']), np.median(a['Av']), np.median(a['GB_p'][a['GB_p'] > 0]),
                np.median(a['GB_OH']), np.median(a['vexp_predicted']), np.median(a['teff']), np.median(a['tinner']),
                np.median(a['odep']), np.median(a['mdot']), np.median(a['rgd'])]
    a.add_row(averages)
